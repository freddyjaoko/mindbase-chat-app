...
function getLanguageModel(model: string) {
    ...
    switch (provider) {
        ...
        default: return openai(model); // Attempt fallback or error
    }
}
...
export async function generateDailyBriefing(profileId: string, tenantId: string) {
    ...
    const recentConversations = await db.query.conversations.findMany({
        where: (conversations, { eq, and, gt }) => and(
            eq(conversations.profileId, profileId),
            gt(conversations.updatedAt, oneDayAgo)
        ),
        with: {
            messages: {
    ...
    const allMessagesText = recentConversations.flatMap(c =>
        c.messages.filter(m => m.content).map(m => `${m.role}: ${m.content}`)
    ).join("\n\n");
